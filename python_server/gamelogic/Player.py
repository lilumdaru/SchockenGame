from gamelogic.MyEnums import PlayerState  # pylint: disable=import-error
...
class Player:
    ...
    def calc_harte_points(self):
        ...
        if self.dices.count(1) == 3:  # 1 1 1
            ...
            self.dices[0] == self.dices[1] and self.dices[0] == self.dices[2]
        ):  # a = b = c ;  e.g. 2 2 2
            harte = 3
            points = 800 + self.dices[0]
        elif (
            self.dices.count(4) == 1
            and self.dices.count(5) == 1
            and self.dices.count(6) == 1
        ):  # 4 5 6
            harte = 2
            points = 704
        elif (
            self.dices.count(3) == 1
            and self.dices.count(4) == 1
            and self.dices.count(5) == 1
        ):  # 3 4 5
            harte = 2
            points = 703
        elif (
            self.dices.count(2) == 1
            and self.dices.count(3) == 1
            and self.dices.count(4) == 1
        ):  # 2 3 4
            harte = 2
            points = 702
        elif (
            self.dices.count(1) == 1
            and self.dices.count(2) == 1
            and self.dices.count(3) == 1
        ):  # 1 2 3
            harte = 2
            points = 701
